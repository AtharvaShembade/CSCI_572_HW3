    ...
    with open(file_path, 'r') as file:
        for line in file:
            ...
            for i in range(len(words) - 1):
                ...
                term_counts[f"bigram:{bigram}"][doc_id] += 1
...
def reducer(term_counts):
    ...
    with open("selected_bigram_index.txt", "w") as bigram_file:
        for term, doc_counts in term_counts.items():
            ...
            # Write bigrams to respective files
            if term.startswith("bigram:"):
                bigram_file.write(f"{term[7:]}\t{doc_counts_str}\n")
